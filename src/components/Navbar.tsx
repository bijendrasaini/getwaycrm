...
import { Menu, X, ChevronDown, Puzzle, ExternalLink } from "lucide-react";
...
const Navbar = () => {
  ...
  useEffect(() => { setMobileOpen(false); setResourcesOpen(false); }, [location.pathname]);
    ...
    <nav className="nav-dark fixed top-0 left-0 right-0 z-50">
      <div className="container-wide flex items-center justify-between h-16 md:h-[68px]">
        ...
        <div className="hidden xl:flex items-center gap-0.5">
          {mainLinks.map((link) => (
            <Link key={link.path} to={link.path}
              className={`px-3 py-2 rounded-lg text-[13px] font-medium transition-colors ${location.pathname === link.path ? "text-primary-foreground bg-teal/10" : "text-[hsl(200,20%,65%)] hover:text-primary-foreground hover:bg-[hsl(200,25%,13%)]"}`}>
              {link.label}
            </Link>
          ))}
          <div ref={resourcesRef} className="relative">
            <button onClick={() => setResourcesOpen(!resourcesOpen)}
              className={`px-3 py-2 rounded-lg text-[13px] font-medium transition-colors inline-flex items-center gap-1 ${resourcesOpen ? "text-primary-foreground bg-teal/10" : "text-[hsl(200,20%,65%)] hover:text-primary-foreground hover:bg-[hsl(200,25%,13%)]"}`}>
        ...
        <div className="hidden xl:flex items-center gap-3">
          ...
          <Link to="/contact" className="outline-button-hero px-4 py-2 rounded-lg text-sm font-medium">Contact Sales</Link>
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="container-wide py-4 flex flex-col gap-1">
              {mainLinks.map((link) => (
                <Link key={link.path} to={link.path} className="px-4 py-3 rounded-lg text-[hsl(200,20%,65%)] hover:text-primary-foreground hover:bg-[hsl(200,25%,13%)] text-sm font-medium transition-colors">{link.label}</Link>
              ))}
