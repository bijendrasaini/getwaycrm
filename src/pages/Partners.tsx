  ...
  <div className="overflow-hidden">
    <section className="gradient-hero pt-32 pb-20">
      <div className="container-wide text-center">
        <AnimatedSection>
          <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase mb-6 bg-electric/10 text-electric-glow border border-electric/20">Partner Network</span>
          ...
          {partnerTypes.map((p, i) => (
            <AnimatedSection key={p.title} delay={i * 0.1}>
              <div className="glass-card-hover rounded-2xl p-8 h-full">
                ...
                <p className="text-[hsl(220,20%,55%)] text-sm leading-relaxed mb-5">{p.desc}</p>
                <ul className="space-y-2">
                  {p.benefits.map((b) => (
                    <li key={b} className="text-[hsl(220,20%,65%)] text-sm flex items-center gap-2">
                      <CheckCircle2 size={14} className="text-electric shrink-0" /> {b}
                    </li>
                  ))}
                </ul>
              </div>
