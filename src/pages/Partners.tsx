...
import { Users, Share2, Building2, Briefcase, CheckCircle2, Send, MessageCircle, ArrowRight, Star, TrendingUp, Zap, Globe } from "lucide-react";
...
const whyPartner = [
  { icon: TrendingUp, title: "Revenue Opportunity", desc: "Build a recurring SaaS revenue stream with competitive margins and lifetime commissions." },
  { icon: Zap, title: "Technology Platform", desc: "Get access to a fully built AI automation platform — no development cost, no infrastructure management." },
  { icon: Globe, title: "Growing Ecosystem", desc: "Join 150+ partners across India building businesses on GETWAY's AI infrastructure." },
  { icon: Star, title: "Partner Benefits", desc: "Dedicated support, marketing materials, training, co-branding, and priority access to new features." },
];

const partnerStats = [
  { number: "150+", label: "Active White-Label Partners" },
  { number: "20,000+", label: "Businesses on Platform" },
  { number: "20+", label: "Years in Technology" },
  { number: "550+", label: "Institutions Powered" },
];

const businessProfiles = [
  "Business Owner", "IT Company", "Digital Marketing Agency",
  "Consultant / Freelancer", "Startup Founder", "Other",
];

const interestAreas = [
  "Channel Partner Program", "White-Label Reseller Program",
  "Affiliate & Referral Program", "GETWAY AI CRM Demo", "CPaaS Communication Platform",
];
...
const Partners = () => {
  ...
  const [form, setForm] = useState({
    fullName: "", company: "", mobile: "", email: "", city: "",
    businessProfile: "", interestArea: "", notes: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.functions.invoke("handle-form-submission", {
        body: {
          name: form.fullName, company: form.company, mobile: form.mobile,
          email: form.email, city: form.city, message: form.notes,
          source: "Website Partner", businessProfile: form.businessProfile, interestArea: form.interestArea,
        },
      });
    ...
    <div className="overflow-hidden">
      ...
      <section className="gradient-hero pt-32 pb-20 relative">
        ...
        <div className="container-wide text-center relative z-10">
          <AnimatedSection>
            <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase mb-6 bg-teal/10 text-teal-light border border-teal/20">White-Label Partner Program</span>
            <h1 className="heading-xl gradient-text-white mb-6">Build Your Own SaaS<br />Business with GETWAY</h1>
            <p className="text-[hsl(200,20%,55%)] text-body-lg max-w-2xl mx-auto mb-8">
              Join 150+ partners worldwide building businesses on GETWAY's AI automation infrastructure. Launch your own branded AI platform with zero development cost.
            </p>
            <button onClick={scrollToForm} className="glow-button px-8 py-3.5 rounded-xl text-primary-foreground font-semibold inline-flex items-center gap-2 text-base">
              Become a White-Label Partner <ArrowRight size={18} />
            </button>
          </AnimatedSection>
        </div>
      </section>
      ...
      <section className="gradient-section-alt section-padding">
        <div className="container-wide">
          <SectionHeader badge="Why Partner" title="Why Become a GETWAY Partner" description="Build a profitable SaaS business backed by world-class AI automation technology." />
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {whyPartner.map((w, i) => (
              <AnimatedSection key={w.title} delay={i * 0.1}>
                <div className="glass-card-hover rounded-2xl p-6 h-full text-center">
                  <div className="feature-icon-box mx-auto mb-4"><w.icon size={22} className="text-teal-light" /></div>
                  <h3 className="font-display font-semibold text-primary-foreground mb-2">{w.title}</h3>
                  <p className="text-[hsl(200,20%,50%)] text-sm">{w.desc}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
      ...
      <section id="partner-form" className="gradient-section-alt section-padding">
        <div className="container-wide max-w-2xl mx-auto">
          <SectionHeader badge="Apply Now" title="GETWAY AI CRM Partner Program" description="Build Your Own SaaS Business with GETWAY" />
          <AnimatePresence mode="wait">
            {submitted ? (
              ...
              <motion.form key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="glass-card rounded-2xl p-8 md:p-10 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {businessProfiles.map((bp) => (
                      <button key={bp} type="button" onClick={() => setForm({ ...form, businessProfile: bp })}
                        className={`px-3 py-2.5 rounded-xl text-xs font-medium border transition-colors ${form.businessProfile === bp ? "bg-teal/20 border-teal/50 text-teal-light" : "bg-[hsl(200,30%,8%)] border-[hsl(200,25%,16%)] text-[hsl(200,20%,55%)] hover:border-teal/30"}`}>
                        {bp}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {interestAreas.map((ia) => (
                      <button key={ia} type="button" onClick={() => setForm({ ...form, interestArea: ia })}
                        className={`px-3 py-2.5 rounded-xl text-xs font-medium border transition-colors text-left ${form.interestArea === ia ? "bg-teal/20 border-teal/50 text-teal-light" : "bg-[hsl(200,30%,8%)] border-[hsl(200,25%,16%)] text-[hsl(200,20%,55%)] hover:border-teal/30"}`}>
                        {ia}
                      </button>
                    ))}
                  </div>
                ...
                <button type="submit" className="glow-button w-full py-4 rounded-xl text-primary-foreground font-semibold inline-flex items-center justify-center gap-2 text-base">
