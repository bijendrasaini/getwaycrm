...
import { GraduationCap, Stethoscope, DollarSign, ShoppingCart, Building2, Briefcase, ArrowRight } from "lucide-react";
...
const industries = [
  ...
  { icon: Stethoscope, title: "Healthcare", desc: "Streamline patient engagement, appointment scheduling, reminders, and compliance workflows for clinics, hospitals, and health networks.", features: ["Patient engagement", "Appointment scheduling", "HIPAA-compliant messaging", "Health records integration"] },
  ...
  <div className="overflow-hidden">
    <section className="gradient-hero pt-32 pb-20">
      <div className="container-wide text-center">
        <AnimatedSection>
          <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase mb-6 bg-electric/10 text-electric-glow border border-electric/20">Solutions</span>
          ...
          {industries.map((ind, i) => (
            <AnimatedSection key={ind.title} delay={i * 0.08}>
              <div className="glass-card-hover rounded-2xl p-8 h-full">
                <div className="feature-icon-box mb-5"><ind.icon size={24} className="text-electric-glow" /></div>
                <h3 className="font-display font-semibold text-primary-foreground text-xl mb-3">{ind.title}</h3>
                <p className="text-[hsl(220,20%,55%)] text-sm leading-relaxed mb-5">{ind.desc}</p>
                <ul className="space-y-2">
                  {ind.features.map((f) => (
                    <li key={f} className="text-[hsl(220,20%,65%)] text-sm flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-electric" /> {f}
                    </li>
                  ))}
                </ul>
              </div>
